...
const PrimarySruveyForm = (props: any) => {
        ...
        if(!isCustom) {
            ...
            categories.map((category: any) => {
                if(category.is_active){
                    categoryOptions.push({ id: optionId, value: category.category_id, label: category.name })
                    optionId++;
                }
            })
            setOptions(categoryOptions)
            props.onOptionChange(categoryOptions)
        }
        setIsCustom(!isCustom)
        props.onIsCustomChangeHandler(!isCustom)
    }
    ...
    return <>
        <Container className="bg-light text-dark pt-4 pb-4 text-left form-block">
            <div>
                ...
                {answerType == "checkbox" && <Form.Group className="mb-3" controlId="formBasicAnswerType">
                    <Form.Check
                        key={`is_custom_checkbox`}
                        inline
                        checked={isCustom}
                        type={"checkbox"}
                        name={"is_custom_checkbox"}
                        label={`Custom Input`}
                        id={`is_custom_checkbox`}
                        onChange={onIsCustomChangeHandler}
                    />
...
export default PrimarySruveyForm